pipeline {
  ...
  stages {
    stage('Semgrep_agent') {
      ...
      steps{
        sh 'env && git fetch origin master && python -m semgrep_agent --baseline-ref $BASELINE_BRANCH --publish-token $SEMGREP_APP_TOKEN --publish-deployment $SEMGREP_DEPLOYMENT_ID'
      }
   }
  }
}
